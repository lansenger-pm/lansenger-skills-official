...
[![Version](https://img.shields.io/badge/version-1.11.0-blue)](https://github.com/lansenger-pm/lansenger-skills-official)

## 甚麼是 Skills？

Skills 是結構化 Markdown 文件，教導 AI Agent 如何使用 `lansenger` CLI 及 SDK。每個 Skill 涵蓋一個業務領域（訊息、行事曆、群組等），包含：
...
| `api_gateway_url` | **必填** | API 網關地址（無預設值，必須提供） | `LANSENGER_API_GATEWAY_URL` |
| `passport_url` | 需 OAuth2 時填 | OAuth2 授權頁地址（需 OAuth2 時必填） | `LANSENGER_PASSPORT_URL` |
...
# api_gateway_url 無預設值，必須手動設定
# lansenger config set api_gateway_url YOUR_GATEWAY_URL

# OAuth2 用戶認證（必須設定 passport_url）
